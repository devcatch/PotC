...
void BLI_SetObjectData()
{
	...
	objLandInterface.CommandTextures.list.t1.xsize = 8;// GR change //boal fix 4 to 8
    ...
    objIconsNote.1x4 = LanguageConvertString(idLngFile, "go_loanshark"); //Greatzen //boal
	...
	if(group=="reload")
	{
		...
		if(i<n)
		{
			...
			if(i>=0)
			{
				...
				{
					Log_SetStringToLog(locations[i].id.label);
				}
			}
		}
