...
void InitBattleInterface()
{
	...
	procLoadIntoNew(); // œÓËÌËÚËÏ Ú‡·ÎËˆÛ ‡ÍÚË‚Ì˚ı ÔÂÍÓ‚
...
void BI_LaunchCommand()
{
	...
	case "BI_SailTo":
		if(targetNum==-1)
		{ // ÔËÔÎ˚Ú¸ ‚ ÎÓÍ‡ÚÓ Ò ËÏÂÌÂÏ locName
			SeaAI_SailToLocator(locName);
		}
		else
		{ // ‰Ó„Ì‡Ú¸ ÔÂÒ‡ Ò ËÌ‰ÂÍÒÓÏ targetNum
...
ref procGetSailTextureData()
{
	...
	if(chrIdx>=0)
	{
		...
		case PIRATE:
			nationFileName = "ships\parus_pirate1.tga"; // boal
...
//boal -->
    int chargeType=GOOD_BALLS;
		...
		case "BI_ChargeCannonballs":
			Log_SetStringToLog("Cannonballs");
			chargeType=GOOD_BALLS;
            Ship_ChangeCharge(GetMainCharacter(), chargeType);
