...
// boal -->
void GeneratePirateQuest(string _sdestination)
{
	ref PChar;
	...
	sdestination = _sdestination;//GenerateDestinationKill();// = _sdestination; not use yet
...
void QuestComplete(string sQuestName)
{
	...
	switch(sQuestName)
	{
		...
		case "Story_leavingOxbay":
			...
			setCharacterShipLocation(characterFromID("Clair Larrouse"), "Muelle_port"); // boal квест про рудники
...
// boal			CloseQuestHeader("Where_are_i");
			bQuestDisableMapEnter = false;
		break;
...
//boal -->
		...
		case "prepare_for_kill_pirate":
            ...
            characters[GetCharacterIndex("Quest Pirate")].chr_ai.hp = 80.0;
			...
			Group_SetAddress("Story_Pirate", Characters[GetMainCharacterIndex()].location, "Quest_ships", "Quest_ship_12");
