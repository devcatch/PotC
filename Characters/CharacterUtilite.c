...
int GetCharacterSkill(ref _refCharacter, string skillName)
{
	...
	if (SKILL_SNEAK == skillName &&	GetCharacterItem(_refCharacter, "jewelry9")>0 && skillN <10)
	{
       skillN++;
	}
	// itmname_CursedCoin		{Монета ацтеков} -1 удача
	if (SKILL_SNEAK == skillName &&	GetCharacterItem(_refCharacter, "CursedCoin")>0 && skillN >0)
	{
       skillN--;
	}
	// Emerald Pendant +1 удача
	if (SKILL_SNEAK == skillName &&	GetCharacterItem(_refCharacter, "jewelry15")>0 && skillN <10)
	{
       skillN++;
	}
	// Pikachuatl Idol +1 удача
	if (SKILL_SNEAK == skillName &&	GetCharacterItem(_refCharacter, "indian7")>0 && skillN <10)
	{
       skillN++;
	}
	// itmname_jewelry8	{Bronze Ring} +1 удача
	if (SKILL_SNEAK == skillName &&	GetCharacterItem(_refCharacter, "jewelry8")>0 && skillN <10)
	{
       skillN++;
	}
	// Weird Animal Figurine (-1 авторитет)
	if (SKILL_LEADERSHIP == skillName &&	GetCharacterItem(_refCharacter, "indian2")>0 && skillN > 0)
	{
       skillN--;
	}
	// itmname_jewelry4	{Изумруд} (+1 авторитет)
	if (SKILL_LEADERSHIP == skillName &&	GetCharacterItem(_refCharacter, "jewelry4")>0 && skillN < 10)
	{
       skillN++;
	}
	// Small Mosaic (+1 торговля)
	if (SKILL_COMMERCE == skillName &&	GetCharacterItem(_refCharacter, "indian3")>0 && skillN <10)
	{
       skillN++;
	}
	// indian6	{Бог войны} (+1 фехтование)
	if (SKILL_FENCING == skillName &&	GetCharacterItem(_refCharacter, "indian6")>0 && skillN <10)
	{
       skillN++;
	}
	// boal <--
	...
	if( CheckCharacterPerk(_refCharacter,"SharedExperience") )
	{
		...
		/*for(int i=1; i<4; i++)
		{
			...
			{	AddCharacterExp(GetCharacter(cn),(_exp+4)/5);
                Log_SetStringToLog("GetOfficers exp " + (_exp+4)/5);
			}
		}*/
//boal -->
        for(int i=1;i<=GetPassengersQuantity(_refCharacter);i++)
        {
           cn = GetPassenger(_refCharacter,i);
           if(cn!=-1)
           {
              if(IsOfficer(GetCharacter(cn)) || IsCompanion(GetCharacter(cn)))
              {
                AddCharacterExp(GetCharacter(cn),(_exp+4)/5);
                //if ((_exp+4)/5 > 0 )
                //{
              ...
              { AddCharacterExp(GetCharacter(cn),(_exp+4)/10); }
           }
        }
// boal <--
	}
