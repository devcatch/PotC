...
void ProcessSave()
{
	...
	string sSaveDescriber = locLabel+"@"+GetOfficersIndex(PChar,0)+"@"+GetOfficersIndex(PChar,1)+"@"+GetOfficersIndex(PChar,2)+"@"+GetOfficersIndex(PChar,3)+"@"+GetStringTime(GetTime())+"  "+GetStringDate(GetDataYear(),GetDataMonth(),GetDataDay());
...
void SettingDataForSave(string saveFileName)
{
	...
	{
		if( !ParseSaveData(IGetSaveStringFromScroll(), &pic1,&pic2,&pic3,&pic4, &locationStr, &timeStr) )
			ParseSaveData("none@-1@-1@-1@-1@not implemented", &pic1,&pic2,&pic3,&pic4, &locationStr, &timeStr);
...
bool ParseSaveData(string fullSaveData, ref pic1,ref pic2,ref pic3,ref pic4, ref locationStr, ref timeStr)
{
	string lastStr;
