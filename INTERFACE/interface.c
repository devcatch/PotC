...
void LaunchBoalDebugScreen()
{
	if(procInterfacePrepare(INTERFACE_BOAL_DEBUG))
	{
		nPrevInterface = -1;
		CurrentInterface = INTERFACE_BOAL_DEBUG;
		InitInterface(Interfaces[CurrentInterface].IniFile);
	}
}
// boal 29.08 debug info <--
...
void LaunchSelectMenu()
{
	...
	// boal cabin -->
	if( bSeaActive && bAbordageStarted && !bCabinStarted) bDisable = true;
	// boal cabin <--
	if( bDisable )
	{
