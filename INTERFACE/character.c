...
{
	//BEGIN MOD TO MOD AdmiralWestside
	int i,cn,ci;
	int CmpQ = 3;
	int PrtQ = 0;
	int PsgQ = 0;
	int OffQ = 3; //GetOfficersQuantity(xi_refMainChar);
	int CmpOffQ[4];
	...
	for(i=1;i<4;i++) 
	{
		...
		if(ci > -1) 
		{
			CmpOffQ[i] = GetOfficersQuantity(GetCharacter(ci));
		}
	}
...
StartOver:
	...
	else
	{
		...
		else
		{
			...
			if ( IsOfficer(GetCharacter(cn)) ) goto StartOver;
		}
	}
	if( cn<0 ) goto StartOver;
//END MOD TO MOD
  ...
  /* END MOD : Stone-D 19/07/2003; Modified by Viper 31/08/2003 */
